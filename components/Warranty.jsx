...
const Warranty = () => {
  ...
  return (
    <div className="custom-container py-8 flex md:flex-row flex-col gap-4">
      <div className="xl:w-[30%] lg:w-[40%] w-full flex flex-col gap-3">
        <p className="text-yellow font-semibold">{t("warranty.subtitle")}</p>
        <h1 className="text-[42px] text-blue font-barlowBlack">{t("warranty.title")}</h1>
          ...
          {warrantyData.map((data) => {
            return (
              <div key={data.id} className="lg:p-2 p-0 outline-none">
                <Image src={data.photo} alt={data.name} width={500} height={500} className="w-full h-full"/>
              </div>
            )
          })}
        </Slider>
